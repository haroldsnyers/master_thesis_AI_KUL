...
model_path = 'output/tfbanformer/TF_to_region_hvg/' + post_fix + '/'
dashboard_model.load_weights(device, best=0, start_directory=start_directory, trained_model_dir=model_path)
...
path_dir = start_directory + 'results/' + '-'.join(model_path.split('/')[1:-1])
...
path =  r"{}cbust_bash.sh".format(start_directory)
...
with open(path_dir + "/" + cbust_type + '_region_ISM_cbust.json') as json_file:
...
with PdfPages('result/in_silico_' + '-'.join(model_path.split('/')[1:-1]) + "-" + data_type + '.pdf') as pdf:
    """examples 2 TFs"""
    # TF_list = [(724, 250)]
